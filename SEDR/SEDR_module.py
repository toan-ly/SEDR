...
class SEDR_no_GCN_module(nn.Module):
    # Copy SEDR but Replace GCN with linear layer
    def __init__(
            self,
            input_dim,
            feat_hidden1=64,
            feat_hidden2=16,
            linear_hidden=16, # Dimension of the GCN output
            p_drop=0.2,
            alpha=1.0,
            dec_clsuter_n=10,
    ):
        ...
        self.alpha = alpha
        self.dec_cluster_n = dec_clsuter_n
        self.latent_dim = self.linear_hidden + self.feat_hidden2 # Update latent dimension
        ...
        # Linear layer to replace VGAE
        self.linear_layer = nn.Linear(self.feat_hidden2, self.linear_hidden)
    ...
    def encode(self, x, adj):
        feat_x = self.encoder(x)
        # hidden1 = self.gc1(feat_x, adj)
        linear_output = self.linear_layer(feat_x)
        return linear_output, feat_x
    ...
    def forward(self, x, adj):
        ...
        linear_output, feat_x = self.encode(x, adj)
        # gnn_z = self.reparameterize(mu, logvar)
        z = torch.cat((feat_x, linear_output), 1)
        de_feat = self.decoder(z, adj)
        ...
        return z, de_feat, q, feat_x, linear_output, loss
...
class SEDR_impute_module_no_GCN(nn.Module):
    def __init__(
            self,
            input_dim,
            feat_hidden1=64,
        ...
        super(SEDR_impute_module_no_GCN, self).__init__()
        self.input_dim = input_dim
        self.feat_hidden1 = feat_hidden1
        self.feat_hidden2 = feat_hidden2
        # self.gcn_hidden1 = gcn_hidden1
        ...
        self.alpha = alpha
        self.dec_cluster_n = dec_clsuter_n
        self.latent_dim = self.linear_hidden + self.feat_hidden2 # Updated latent size
